...
/**
 * Minimal Chromium DevTools helpers inspired by Mario Zechner's
 * "What if you don't need MCP?" article.
 *
 * Keeps everything in one TypeScript CLI so agents (or humans) can drive Chromium browsers
...
const DEFAULT_CHROMIUM_BIN = '/Applications/Chromium.app/Contents/MacOS/Chromium';

function browserURL(port: number): string {
  return `http://localhost:${port}`;
}

function inferBrowserName(chromiumPath: string): string {
  const name = path.basename(chromiumPath);
  return name || 'Chromium';
}

function inferProfileRoot(chromiumPath: string): string {
  const appBundle = path.basename(path.dirname(path.dirname(path.dirname(chromiumPath))));
  const appName = appBundle.endsWith('.app') ? appBundle.slice(0, -4) : appBundle;
  ...
  return path.join(os.homedir(), 'Library', 'Application Support', 'Chromium');
}

async function ensureProfileDirFresh(profileDir: string, browserName: string) {
  const markerPath = path.join(profileDir, 'last-browser.txt');
  try {
    const lastBrowser = (await readFile(markerPath, 'utf8')).trim();
    if (lastBrowser && lastBrowser !== browserName) {
      const entries = await readdir(profileDir);
      await Promise.all(entries.map((entry) => rm(path.join(profileDir, entry), { recursive: true, force: true })));
    }
  } catch {
    // ignore missing marker or unreadable dir
  }
  await writeFile(markerPath, `${browserName}\n`);
}
...
program
  .name('browser-tools')
  .description('Lightweight Chromium DevTools helpers (no MCP required).')
  .configureHelp({ sortSubcommands: true })
  .showSuggestionAfterError();

program
  .command('start')
  .description('Launch Chromium with remote debugging enabled.')
  .option('-p, --port <number>', 'Remote debugging port (default: 9222)', (value) => Number.parseInt(value, 10), DEFAULT_PORT)
  .option('--profile <name>', 'Copy a Chromium profile before launch (e.g., "Default", "Profile 2").')
  .option('--profile-dir <path>', 'Directory for the temporary Chromium profile.', DEFAULT_PROFILE_DIR)
  .option('--no-fresh-tabs', 'Do not clear restored session tabs after copying the profile.')
  .option('--chromium-path <path>', 'Path to the Chromium binary.', DEFAULT_CHROMIUM_BIN)
  .option('--kill-existing', 'Stop any running Chromium before launch (default: false).', false)
  .action(async (options) => {
    const { port, profile, profileDir, chromiumPath, killExisting, freshTabs } = options as {
      port: number;
      profile?: string;
      profileDir: string;
      chromiumPath: string;
    ...
    const browserName = inferBrowserName(chromiumPath);

    if (killExisting) {
      try {
        execSync(`killall '${browserName}'`, { stdio: 'ignore' });
      } catch {
    ...
    await ensureProfileDirFresh(profileDir, browserName);
    if (profile) {
      const baseDir = inferProfileRoot(chromiumPath);
      const sourceProfile = path.join(baseDir, profile);
      try {
        execSync(`test -d "${sourceProfile}"`);
      } catch {
        console.error(`✗ Chromium profile not found: ${sourceProfile}`);
        process.exit(1);
      }

      const localState = path.join(baseDir, 'Local State');
      execSync(`mkdir -p "${profileDir}"`);
      execSync(`rsync -a "${localState}" "${profileDir}/"`, { stdio: 'ignore' });
      const targetProfileDir = path.join(profileDir, profile);
      execSync(`rsync -a "${sourceProfile}/" "${targetProfileDir}/"`, { stdio: 'ignore' });
      if (freshTabs) {
        const sessionsDir = path.join(targetProfileDir, 'Sessions');
        const sessionFiles = ['Current Session', 'Last Session', 'Current Tabs', 'Last Tabs'];
        sessionFiles.forEach((file) => {
          try {
            execSync(`rm -f "${path.join(targetProfileDir, file)}"`);
          } catch {
            // ignore missing files
          }
        });
        try {
          execSync(`rm -rf "${sessionsDir}"`);
        } catch {
    ...
    if (!connected) {
      console.error(`✗ Failed to start ${browserName} on port ${port}`);
      process.exit(1);
    }
    const profileLabel = profile ? ` (profile copied: ${profile})` : '';
    console.log(`✓ ${browserName} listening on http://localhost:${port}${profileLabel}`);
  });
...
program
  ...
  .action(async (url: string, options) => {
    ...
    const parsedTimeout = options.timeout as number;
    const timeoutSeconds = Number.isFinite(parsedTimeout) ? parsedTimeout : 15;
    const timeoutMs = Math.max(3, timeoutSeconds) * 1000;
    const targetUrl = `http://localhost:${port}/json/new?${encodeURIComponent(url)}`;
    const activateUrl = (targetId: string) => `http://localhost:${port}/json/activate/${targetId}`;
    if (options.new) {
      const created = (await fetchJsonWithMethod(targetUrl, 'PUT', timeoutMs)) as { id?: string; url?: string };
      if (created?.id) {
        await fetchJsonWithMethod(activateUrl(created.id), 'PUT', timeoutMs).catch(() => {});
      }
      console.log('✓ Opened in new tab:', created?.url ?? url);
      return;
    }

    const targets = (await fetchJson(`http://localhost:${port}/json/list`, timeoutMs)) as ChromeTabInfo[] | undefined;
    const hasUserPage = Array.isArray(targets)
      ? targets.some((tab) => {
    ...
    try {
      ...
      console.warn(`⚠️  Current-tab navigation failed, opening a new tab instead: ${message}`);
      const created = (await fetchJsonWithMethod(targetUrl, 'PUT', timeoutMs)) as { id?: string; url?: string };
      if (created?.id) {
        await fetchJsonWithMethod(activateUrl(created.id), 'PUT', timeoutMs).catch(() => {});
      }
...
program
  .command('inspect')
  .description('List Chromium processes launched with --remote-debugging-port and show their open tabs.')
  ...
  .action(async (options) => {
    ...
    if (sessions.length === 0) {
      console.log('No Chromium instances with DevTools ports found.');
      return;
    }
    sessions.forEach((session, index) => {
      ...
      const header = [`Browser PID ${session.pid}`, `(${transport})`];
...
program
  .command('kill')
  .description('Terminate Chromium instances that have DevTools ports open.')
  ...
  .option('--all', 'Kill every matching Chromium instance.', false)
  ...
  .action(async (options) => {
    ...
    if (sessions.length === 0) {
      console.log('No matching Chromium instances found.');
      return;
    }
    if (!options.force) {
      console.log('About to terminate the following Chromium sessions:');
    ...
    sessions.forEach((session) => {
      try {
        ...
        const transport = session.port !== undefined ? `port ${session.port}` : session.usesPipe ? 'debugging pipe' : 'unknown transport';
        console.log(`✓ Killed Browser PID ${session.pid} (${transport})`);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
async function extractReadableContent(page: any): Promise<{ title?: string; content?: string; url: string }> {
  ...
  const result = await page.evaluate(`(() => {
    window.__name = (_name, value) => value;
    const asMarkdown = (html) => {
      if (!html) return '';
      const TurndownService = window.TurndownService;
      const turndownPluginGfm = window.turndownPluginGfm;
      if (!TurndownService) {
        return '';
      }
    ...
    const fallbackText = () => {
      ...
      return (main && main.textContent ? main.textContent.trim() : '') || '';
    };
    ...
    try {
      ...
      if (Readability) {
        ...
        content = asMarkdown(article && article.content) || (article && article.textContent) || '';
      }
    } catch {
...
async function listDevtoolsChromes(): Promise<ChromeProcessInfo[]> {
  if (process.platform !== 'darwin' && process.platform !== 'linux') {
      console.warn('Chromium inspection is only supported on macOS and Linux for now.');
    return [];
  }
  let output = '';
  try {
  ...
  output
    ...
    .forEach((line) => {
      ...
      if (!/--remote-debugging-port/.test(command) && !/--remote-debugging-pipe/.test(command)) {
        return;
      }
...
function fetchJsonWithMethod(url: string, method: 'GET' | 'PUT', timeoutMs = 2000): Promise<unknown> {
  return new Promise((resolve, reject) => {
