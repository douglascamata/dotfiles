/**
 * Automated Code Review Extension (inspired by Codex's review feature)
 *
 * Provides a `/auto-review` command that prompts the agent to review code changes.
 ...
 *
 * Usage:
 * - `/auto-review` - show interactive selector
 * - `/auto-review pr 123` - review PR #123 (checks out locally)
 * - `/auto-review pr https://github.com/owner/repo/pull/123` - review PR from URL
 * - `/auto-review uncommitted` - review uncommitted changes directly
 * - `/auto-review branch main` - review against main branch
 * - `/auto-review commit abc123` - review specific commit
 * - `/auto-review folder src docs` - review specific folders/files (snapshot, not diff)
 * - `/auto-review custom "check for security issues"` - custom instructions
 *
...
// This is intentional - the UI and /end-auto-review command assume a single active review.
...
function setReviewWidget(ctx: ExtensionContext, active: boolean) {
	...
	ctx.ui.setWidget("review", (_tui, theme) => {
		const text = new Text(theme.fg("warning", "Review session active, return with /end-auto-review"), 0, 0);
		return {
			render(width: number) {
...
export default function automatedCodeReviewExtension(pi: ExtensionAPI) {
	...
	async function executeReview(ctx: ExtensionCommandContext, target: ReviewTarget, useFreshSession: boolean): Promise<void> {
		...
		if (reviewOriginId) {
			ctx.ui.notify("Already in a review. Use /end-auto-review to finish first.", "warning");
			return;
		}
	...
	// Register the /auto-review command
...
`;
	...
	// Register the /end-auto-review command
	pi.registerCommand("end-auto-review", {
		...
		handler: async (args, ctx) => {
			...
			if (!reviewOriginId) {
				...
				if (state?.active && state.originId) {
					...
					ctx.ui.notify("Not in a review branch (use /auto-review first, or review was started in current session mode)", "info");
					return;
				}
			}
			...
			if (summaryChoice === undefined) {
				// User cancelled - keep state so they can call /end-auto-review again
				ctx.ui.notify("Cancelled. Use /end-auto-review to try again.", "info");
				return;
			}
			...
			if (wantsSummary) {
				...
				if (result === null) {
					...
					ctx.ui.notify("Summarization cancelled. Use /end-auto-review to try again.", "info");
					return;
				}
				...
				try {
					...
					if (result.cancelled) {
						...
						ctx.ui.notify("Navigation cancelled. Use /end-auto-review to try again.", "info");
						return;
					}
